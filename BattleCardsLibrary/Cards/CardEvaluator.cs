...
public class CreateCard
{
    public static Card CardCreator(string[] text)//text viene de aplicarle Split('') a string inicial
    {
        ...
        {
            case "Monster":
                return new MonsterCard(propertiesToSet, int.Parse(text[text.Length - 1].Remove(text[text.Length - 1].Length - 1, 1).Remove(0, 1)));

            case "Spell":
                return new SpellCard(propertiesToSet, int.Parse(text[text.Length - 1].Remove(text[text.Length - 1].Length - 1, 1).Remove(0, 1)));
...
public class TernaryExpression : Expression
{
    ...
    public TernaryExpression(string ternaryExpression)
    {
        ...
        Condition = Interpreter.BuildExpression(SeparatedExpressions[0]);
        IfTrue = Interpreter.BuildExpression(SeparatedExpressions[1]);
        Else = Interpreter.BuildExpression(SeparatedExpressions[2]);
    }
...
public abstract class BinaryExpression : Expression
{
    ...
    {
        Left = Interpreter.BuildExpression(left);
        Right = Interpreter.BuildExpression(right);
    }
