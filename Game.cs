...
public class Game //Asumir que la informacion me va a entrar po alguna via, tu solo vas a trabajar con ella.Olvidate de por donde entre.
{
    public static List<Player> Players;
       ...
       Array.Copy(arr, 0, a,firstPos,arr.Length);
        ...
        {
            if (player.ManaPoints < card.Cost) throw new Exception("No tienes suficiente mana.");
        ...
        public static void Activate(Card card, Player player)
        {
            if (card is Unit)
            {
                if (player.ManaPoints < (card as Unit).Magic) throw new Exception("No tienes suficente mana");
                player.ManaPoints -= (card as Unit).Magic;
            }
            else player.ManaPoints -= card.Cost;
            card.Effect();
        }
        ...
        public void Turn(Player player, Board board)
        {
            ...
            List<SpellCards> SpellsOnBoard = GetSpellCardsOnBoard(Board board);
        ...
        {
            if (player1.LifePoints <= 0 || player1.Deck.Count == 0)
            {
                player2.Winner = true;
                return true;
            }
            if (player2.LifePoints <= 0 || player2.Deck.Count == 0)
            {
                player1.Winner = true;
                return true;
            }
    ...
    public static void SetGamePlayers(List<Player> Players, string name1, string name2)
    {
        ...
        foreach (var name in names)
        {
            ...
            {
                case "Seamus-Virtual":
                    VirtualPlayerSeamus P1 = new VirtualPlayerSeamus(name);
                    Players.Add(P1);
                    break;
    ...
    public static void CreateDeck(Player player, int cardsInDeck)
    {
